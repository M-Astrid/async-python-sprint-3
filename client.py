...
class Client:
    async def connect(self, host: str, port: int, username: str):
        ...
        req = Request(
            "POST", "/connect", "HTTP/1.1", {"Host": f"{host}:{port}"}, data
        )
    ...
    @staticmethod
    async def _receive_messages(reader: StreamReader):
        while True:
            ...
            if message.is_system:
                print(Fore.YELLOW, end="")
            print(
                (f"{message.from_username}: " if message.from_username else "")
                + message.data.strip()
                + Style.RESET_ALL
            )

    @staticmethod
    async def _send_messages(writer: StreamWriter, from_username: str):
        while True:
            data = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--server-url",
        dest="server",
        help="chat server address in format <host>:<port>",
        required=True,
        metavar="<host>:<port>",
    )
    parser.add_argument(
        ...
        metavar="command",
        type=str,
        help=f"a command to execute {COMMANDS}",
        choices=COMMANDS,
    )
    ...
    parser.add_argument(
        ...
        metavar="<your_name>",
        required=bool({"send_all", "send_private"} & set(sys.argv)),
    )

    parser.add_argument(
        "--to_username",
        dest="to_username",
        help="enter target user name",
        metavar="<user_name>",
        required="send_private" in sys.argv,
    )
    ...
    match args.command:
        ...
        case "send_private":
            ...
            resp = httpx.post(
                f"http://{host}:{port}/send-private", json=msg.to_dict()
            )
        ...
        case "send_all":
            ...
            resp = httpx.post(
                f"http://{host}:{port}/send-all", json=msg.to_dict()
            )
            print(resp.status_code, resp.text if resp.text else "")
        case "status":
