...
class Chat:
    ...
    async def client_connected(self, req: Request, reader: StreamReader, writer: StreamWriter):
        ...
        try:
            ...
            writer.write(Message(data="Connected to the chat.", is_system=True).to_bytes())
            await writer.drain()
            ...
            while True:
                ...
                try:
                    ...
                    writer.write(Message(data=str(e), is_system=True, is_error=True).to_bytes())
                    await writer.drain()
                    continue
                ...
                if msg.is_private:
                    try:
                        ...
                        writer.write(Message(data=f"User {msg.to_username} not found.", is_system=True, is_error=True).to_bytes())
                        await writer.drain()
                else:
            ...
            writer.write(Message(data="Unexpected error.", is_system=True, is_error=True).to_bytes())

        if username and username in self.clients:
            del self.clients[username]
        writer.write(Message(data="Disconnected from the chat.", is_system=True).to_bytes())
        await writer.drain()
    ...
    async def load_history(self, client: Client):
        if not self.messages:
            client.writer.write(Message(data=f"Chat history empty. Write something!", is_system=True).to_bytes())
            await client.writer.drain()
